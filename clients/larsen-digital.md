...
offer: DTC brand growth and exit planning. M&A team has closed over $1.2B in transactions. Goal: 8-figure exit. 250M+ in Shopify revenue managed.
...
- Lead asks what we do: give a concise human overview (we work with DTC consumer brands to drive growth across Meta, Google, Email and TikTok, and build toward a clean exit through our M&A team, which has closed over $1.2B in transactions). Include a relevant case study link. Do not dump the full pitch.
...
reply_body_rules: |
  ...
  Open with a short plain acknowledgment (one clause is fine). Then: one plain sentence on what we do (we help DTC founders maximize the value of their brand at exit, both by growing the right parts of the business and by working with our M&A team, which has closed over $1.2B in transactions). If the lead mentioned something specific about their brand IN THEIR OWN MESSAGE, you may reference it. Never pull details from website research. Then slot close or case study link if warranted.
  Example shape (no research details used):
    "Sure thing. We work with DTC founders to grow the business in a way that sets up a clean exit, not just growth for growth's sake, but building toward the highest possible valuation. Our M&A team has closed over $1.2B in transactions so we know what buyers actually pay a premium for. Easiest is to grab a time here: https://calendly.com/d/dtm8-3nx-vr9/intro-call-operating-partner"
  ...
  - M&A track record: always say "our M&A team, which has closed over $1.2B in transactions." Never use "$1B+" or "M&A partners" in any reply copy. HARD RULE: never state what KIND of transactions ($1.2B in transactions only, never "consumer transactions", never naming an industry, deal type, or sector).
...
- Never use "$1B+" or "M&A partners" in reply copy. Always use "our M&A team, which has closed over $1.2B in transactions." HARD RULE: never specify what kind of transactions (no "consumer transactions", no industry, no deal type).
...
fu_context: |
  Nicklas Larsen runs Larsen Digital, DTC brand growth with a built-in exit strategy. 250M+ in Shopify revenue managed. M&A team has closed over $1.2B in transactions.
...
Track record of 250M+ in Shopify revenue. Goal is an 8-figure exit. M&A team has closed over $1.2B in transactions. Compensation is fully bespoke and aligned to the business's end goal (growth, profitability, or EV) so incentives stay aligned as we scale.

**What makes them different:**
Specific exit target (8-figure). Deep M&A network (over $1.2B in transactions closed). Compensation aligned to the client's outcome.
...
- M&A team: over $1.2B in transactions closed
...
- How we work / more info: we work with DTC consumer brands on two tracks, growth (paid acquisition, retention, conversion) and exit (our M&A team, which has closed over $1.2B in transactions). The goal is increasing enterprise value before a brand goes to market, not just growing revenue for its own sake. For a sell-side lead, lead with the exit side and do not pitch the growth services. Include a relevant case study and always link it (https://www.larsendigitalmarketing.com/case-studies), never "take a look".
...
- M&A track record phrasing: always "our M&A team, which has closed over $1.2B in transactions." Never the old "$1B+" phrasing. HARD RULE (added 2026-08-28): never state what kind of transactions, no "consumer transactions", no industry or deal type. Just "$1.2B in transactions."
...
We take over multi-channel growth and retention. For context on velocity, last year we took 2 brands past 8 figure run rates. One went from $0 to $850k/mo in 4 months, a global brand you would recognize instantly. Our M&A team has closed over $1.2B in transactions, giving us a clear read on what strategic and PE buyers pay a premium for and how to position your brand for the best possible exit.

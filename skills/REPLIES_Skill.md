# Agency Evolution — Reply & Follow-Up Playbook

This file contains the reply process, objection handling rules, and all 10 follow-up email
templates for each client. Claude should read this alongside `skills/SKILL.md` before
generating, improving, or classifying any reply or follow-up email.

---

## The Process (Same for All Clients)

1. **Lead replies → interested** — send a reply + Calendly link to book a call. Use the client's Calendly link from `SKILL.md`.
...
- Never make price or valuation promises unless the client profile in `SKILL.md` says otherwise
...
- Tone and offer details always come from the client profile in `SKILL.md`
...
| "What's the valuation / deal structure?" | Answer based on client offer in `SKILL.md`, offer a call for details |
...
Tone gets progressively shorter and more direct as the sequence progresses.
...
Used across all clients — swap in client name and Calendly details from `SKILL.md`.

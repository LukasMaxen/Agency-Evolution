...
${reply.message}`;
  ...
  if (!result) {
    // Claude failed — revert to new so it can be handled manually
    await pool.query(`UPDATE replies SET status = 'new' WHERE id = $1`, [replyId]);
    await postToSlack(
  ...
  // Handle flag_unsubscribe
  if (result.flag_unsubscribe) {
    await pool.query(
      `UPDATE replies SET interested = FALSE WHERE id = $1`,
      [replyId]
    );
    await pool.query(
      `UPDATE follow_ups SET meeting_booked = FALSE, next_fu_due = NULL WHERE reply_id = $1`,
      [replyId]
    );
  }
  ...
  if (result.flag_meeting_booked) {
    ...
    await pool.query(
      `UPDATE follow_ups SET meeting_booked = TRUE, next_fu_due = NULL WHERE reply_id = $1`,
      [replyId]
    );
  }
  ...
  if (result.action === "auto_send" && result.reply_body) {
    ...
    if (sent) {
      ...
      // EmailBison send failed — revert and notify
      await pool.query(`UPDATE replies SET status = 'new' WHERE id = $1`, [replyId]);
      await postToSlack(

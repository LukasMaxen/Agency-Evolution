...
// requires both the workspace's instance URL (workspaces table) and a matched reply row's
// EmailBison UUID (email_bison_id — NOT email_bison_reply_id, a different numeric id that
// 404s) — returns null if either is missing.
async function getThreadUrl(workspaceSlug: string, leadEmail: string): Promise<string | null> {
  try {
    ...
    const [ws, rep] = await Promise.all([
      ...
      pool.query(
        `SELECT email_bison_id FROM replies
    ...
    const uuid = rep.rows[0]?.email_bison_id;
    return instanceUrl && uuid ? buildEmailBisonUrl(instanceUrl, uuid) : null;
  } catch {
    return null;
  }
}
